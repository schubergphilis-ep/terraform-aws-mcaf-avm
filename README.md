...
| Input | Names |
| --- | --- |
| `authentication_settings.role_name` | The default workspace's roles. |
| `additional_tfe_workspaces.<name>.override_authentication_settings.role_name` | That one additional workspace's roles. |
| `tfe_project.role_name` | The project-scoped roles. |

Each value replaces the base name in full, and none of them is inherited by the others — so setting one never affects another. IAM role names must be unique within the account, which the defaults above satisfy for every scenario except [scenario 3](#scenario-3-project-scoped-roles-alongside-per-workspace-roles), where the default workspace's roles and the project's roles are created side by side and one of the two names has to be set explicitly.
...
This is the only scenario where the default workspace's roles and the project's roles are created in the same account, and both default to the base name `TFEPipeline`. The module rejects that at plan time, so give one of the two an explicit name — `tfe_project.role_name` above, or `authentication_settings.role_name`.
...
| Name | Version |
| ---- | ------- |
| <a name="provider_aws.account"></a> [aws.account](#provider\_aws.account) | 6.58.0 |
...
| <a name="input_tfe_project"></a> [tfe\_project](#input\_tfe\_project) | TFE project configuration including variable sets and authentication settings. If no name is provided, var.name will be used for the project name & variable set name. `enable_project_scoped_authentication` defaults to true when authentication\_settings.scope is "project"; set it to true while the scope is "workspace" to create project-scoped roles in addition to the per-workspace roles, for example for workspaces created outside of this module. `enabled` defaults to true whenever project-scoped authentication is enabled, since it requires a project, and false otherwise. `role_name` names the project-scoped roles and defaults to "TFEPipeline"; it is independent of `authentication_settings.role_name`, which names the default workspace's roles. | <pre>object({<br/>    enabled = optional(bool) # defaults to true when project-scoped authentication is enabled<br/>    name    = optional(string)<br/><br/>    default_agent_pool_id                = optional(string)<br/>    default_execution_mode               = optional(string)<br/>    enable_project_scoped_authentication = optional(bool)   # defaults to true when authentication_settings.scope is "project"<br/>    role_name                            = optional(string) # names the project-scoped roles, defaults to "TFEPipeline"<br/>    variable_set_ids                     = optional(map(string), {})<br/><br/>    variable_set = optional(object({<br/>      clear_text_env_variables       = optional(map(string), {})<br/>      clear_text_hcl_variables       = optional(map(string), {})<br/>      clear_text_terraform_variables = optional(map(string), {})<br/>    }), {})<br/>  })</pre> | `{}` | no |
